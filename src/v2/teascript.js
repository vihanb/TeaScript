(function (global, data) {
  ...
  var TeaScript = function (code, inputs, opts, plug) {
    ...
    // .replace(/([A-Za-z])([\/#])/g, "$1($2$3") // (
      .replace(/([(,=])#/g,'$1(l,i,a,b)=>')       // =>
      .replace(/(?![\\#]|^)\[(?!")([^\]]+)\]/g,".G($1)")
    // .replace(/([lia_c])(\d)([ :,)+=?\-*\\\/%])/, "$1[$2]$3") // [n]
    // Very long RegExp attempting to handle (most) cases to avoid periods. Will perfect in TeaScript 3
      .replace(/((?:[^A-Za-z]|^)[xyzliaLIASX_]|[\])$/'"])([A-Za-z])(?=[`\[(:.+,)<>?:/ *-=\|]|$)/g,'$1["$2"]')
      .replace(/([abcdfghijklmnopqrstuvwxyz)])([0-9])(?=[(><=),])/gi, '$1[$2]')
    ...
    // Input Variables
    if (opts.ar)  inputs = (/^[\(\[\{][\S\s]+[\)\]\}]$/.test(inputs[0]) ? inputs[0].slice(1, -1) : inputs[0]).split(",");
