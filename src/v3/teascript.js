...
(function (global) {
  const TeaScript = (Code, Data = { rep: {} }) => {
    ...
    const REGEX_CLASS = new Map([["A", "[A-Z]"], ["a", "[a-z]"], ["L", "[A-Za-z]"], ["N", "[A-Za-z0-9]"]]);
      ...
      for (let i = 0; i < Code.length; i++) {
        ...
        /* Disable Definitions?
           ...
           Definition = 1;
            ...
            if (COMMENT.some(Start => Code.slice(i, i + Start[0].length) === Start[0])) {
              // Comment
            ...
            } else if (Code.slice(i).indexOf(`"` + KEY_POLYGLOT) === 0) {
              // Start Polygot
              let CollectedCode = "";
              i++;
              ...
              --i;
            } else if (Code[i] === "/" ? !MATCH_DIV.test([...Code.slice(0, i)].reverse().join("").trim() || "") : ESCAPES_START.includes(Code[i])) {
            ...
            } else if (Data.rep.hasOwnProperty(Code[i])) {
              // Unicode char
    ...
    // Adjusts parenthesis
    {
      ...
      for (let i = 0; i < Code_1.length; i++) {
        if (Code_1[i] === "/" ? !MATCH_DIV.test([...Code_1.slice(0, i)].reverse().join("").trim() || "") : ESCAPES_START.includes(Code_1[i])) {
          // Found an escape character (string)
